...
class _AccountListWidgetState extends State<AccountListWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<GetAccountCubit, GetAccountState>(
      builder: (context, state) {
        if (state is GetAccountSuccess) {
          final accounts = List<Account>.from(state.account);

          if (accounts.isEmpty) {
            return NoDataFoundScreen(
              title: context.tr('noAccAddKey'),
              subTitle: context.tr('accAddDesKey'),
            );
          }

          // 2. Logic to move ID "-2" to the end
          final addAccountIndex = accounts.indexWhere((acc) => acc.id == '-2');

          final addAccountItem = accounts.removeAt(addAccountIndex);
          accounts.add(addAccountItem); // Appends to the end
              ...
              builder: (context, value, child) {
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    // Calculation logic (Only run for real accounts)
                    double totalActualBalance = 0;
                    if (account.id != '-2') {
                      final totalIncome = context.read<GetTransactionCubit>().getTotalIncomeByAccountId(accountId: account.id);
                      final totalExpense = context.read<GetTransactionCubit>().getTotalExpenseByAccountId(accountId: account.id);
                      final totalTransfer = context.read<GetTransactionCubit>().getTotalTransferAmount(accountId: account.id);
                      totalActualBalance = totalIncome - totalExpense + account.amount + totalTransfer;
                    }
                    ...
                    return GestureDetector(
                      ...
                      child: Container(
                        ...
                        margin: EdgeInsetsDirectional.only(
                          end: (index == accounts.length - 1) ? 0 : context.width * 0.03,
                        ),
                        decoration: BoxDecoration(
                          color: colorScheme.surface,
                          borderRadius: BorderRadius.circular(15),
                          border: Border.all(
                            color: (isSelected && (account.id != '-2')) ? colorScheme.primary : colorScheme.surface,
                            width: 2,
                          ),
                        ),
                        child: (account.id == '-2')

...
import 'package:expenseapp/core/constants/app_images.dart';
...
class QImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const errorImg = AppImages.splashScreen;
    ...
    final errorWidget = Image.asset(errorImg, width: width, height: height, fit: fit);

    return SizedBox(
      width: width,
      height: height,
      child: ClipRRect(
        borderRadius: isCircular ? BorderRadius.circular(99999) : BorderRadius.zero,
        child: switch ((isNetworked, isSvg)) {
          ...
          //
          (false, true) => SvgPicture.asset(image, width: width, height: height, colorFilter: colorFilter, fit: fit, alignment: alignment),
          //
          ...
          //
          (true, true) => SvgPicture.network(image, width: width, height: height, colorFilter: colorFilter, fit: fit, alignment: alignment),
        },
      ),
    );
  }
